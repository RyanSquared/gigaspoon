...
def test_email(app):
    no_domain_validator = gs.v.Email("email")
    domain_validator = gs.v.Email("email", domain="example.com")

    @app.route("/no_domain", methods=["GET", "POST"])
    @gs.validator(no_domain_validator)
    @gs.base
    def no_domain(form):
        if form.is_form_mode():
            return "success"
...
def test_ipaddr(app):
    ipv4_validator = gs.v.IPAddress("ipv4")
    ipv6_validator = gs.v.IPAddress("ipv6", address_type=["ipv6"])

    both_validator = gs.v.IPAddress("both", address_type=["ipv4", "ipv6"])

    @app.route("/ipv4", methods=["GET", "POST"])
    @gs.validator(ipv4_validator)
    @gs.base
    def ipv4(form):
        if form.is_form_mode():
            return "success"
...
def test_regex(app):
    regex_validator = gs.v.Regex("username", "^[a-z][a-z0-9]{0,29}$")

    @app.route("/", methods=["GET", "POST"])
    @gs.validator(regex_validator)
    @gs.base
    def index(form):
        if form.is_form_mode():
            return "success"
